...
import { Box, chakra, Flex, Icon, Spacer } from '@chakra-ui/react';
...
const NavItem = (props) => {
  ...
  return (
    <Link href={link} key={menuNumber} passHref={true}>
      <Box className={styles.sidenavItem}>
        ...
        >
          ...
          {hasChild && (
            <>
              <Spacer />
              <Box fontSize='16px' padding='4' transition='all 0.4s' className={toggleChild && styles.iconRotate}>
                <IoIosArrowForward />
              </Box>
            </>
          )}
        ...
        {!collapse && window.innerWidth > 1024 && (
          <Tooltip childItem={childItem} menuNumber={menuNumber} hasChild={hasChild} name={name} />
        )}
      </Box>
    </Link>
  );
